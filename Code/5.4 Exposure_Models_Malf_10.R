# 5.4 Exposure models: PM2.5 en nivel/10 (`*_PM25_*_10`); Levo/K en nivel bruto -----
...
# Períodos: PM2.5 como original/10; Levo/K según nivel bruto (<period>_Levo_<cs|sp>)
...
# Predictores válidos: PM25 con sufijo _10; Levo/K bruto (sin _10 ni iqr_)
...
writexl::write_xlsx(
  combinations, 
  path = "Output/Models/List_models_exposure_malf_div10.xlsx"
)
...
# Combinamos todos los resultados (conserva `term` por coeficiente en modelos múltiples)
results_logit <- dplyr::bind_rows(results_list)

# Guardamos resultados (archivos específicos /10 para no pisar modelo IQR 5.1)
save(results_list, file = "Output/Models/Exposure_models_malf_div10.RData")

writexl::write_xlsx(
  results_logit,
  path = "Output/Models/Exposure_models_malf_div10.xlsx"
)

# Test de prueba de algunos modelos (/10)
m1 <- glm(
  malf ~ pct1_PM25_cs_10 + t1_PM25_cs_10 + t2_PM25_cs_10 + t3_PM25_cs_10,
...
m2 <- glm(
  malf ~ t1_PM25_cs_10 + t2_PM25_cs_10 + t3_PM25_cs_10 +
    edad_madre + sexo_rn + a_nac + estacion,
  family = binomial(link = logit), data = data_full_wide)
...
pred_pm25_div10_card <- names(data_model_card)[
  grepl("^(pct1|t1|t2|t3|w20|tot)_", names(data_model_card)) &
...
available_predictors_card <- union(pred_pm25_div10_card, raw_levo_k_card)
...
writexl::write_xlsx(
  combinations_card,
  path = "Output/Models/List_models_exposure_malf_card_bin_div10.xlsx"
)
...
save(results_list_card, file = "Output/Models/Exposure_models_malf_card_bin_div10.RData")

writexl::write_xlsx(
  results_logit_card,
  path = "Output/Models/Exposure_models_malf_card_bin_div10.xlsx"
)
