# 5.1 Exposure models PM2.5 por IQR; Levo/K en nivel bruto -----

## Settings ----
source("Code/0.1 Settings.R")
source("Code/0.2 Packages.R")

## 1. Cargamos los datos ----

data_full_wide <- rio::import("Output/Data_malf_exposure_wide.RData")
#data_full_long <- rio::import("Output/Data_malf_exposure_long.RData")
glimpse(data_full_wide)

## 2. Preparamos los datos para modelado ----

# Filtramos datos con valores válidos en variables dependientes
data_model <- data_full_wide |>
  filter(!is.na(malf))

## 3. Definimos variables y creamos grilla de modelos ----

# Variables dependientes
dependent_vars <- c("malf", colnames(data_full_wide)[str_detect(colnames(data_full_wide), pattern = "malf_.*_bin")])
dependent_vars

# Variables de control para modelos ajustados
control_vars <- c("edad_madre", "sexo_rn", "a_nac", "estacion")

test <- data_model |>
  select(all_of(control_vars))

# Períodos de tiempo: PM2.5 en columnas iqr_<period>_PM25_<cs|sp>; Levo/K en nivel bruto
time_periods <- c("pct1", "t1", "t2", "t3", "w20", "tot")

# Contaminantes y tipos
contaminants <- c("PM25", "Levo", "K")
types <- c("cs", "sp")

# nombre de columna exposición en datos: PM25 → iqr_<period>_PM25_<tipo>; Levo/K → nivel bruto
pred_single_iqr <- function(tiempo_lab, contaminante, tipo) {
  if (contaminante == "PM25") {
    paste0("iqr_", tiempo_lab, "_PM25_", tipo)
  } else {
    paste0(tiempo_lab, "_", contaminante, "_", tipo)
  }
}

pred_joint_iqr_pct <- function(contaminante, tipo) {
  pr <- c("pct1", "t1", "t2", "t3")
  paste(
    vapply(pr, pred_single_iqr, character(1),
           contaminante = contaminante, tipo = tipo),
    collapse = " + "
  )
}

pred_joint_iqr_t123 <- function(contaminante, tipo) {
  pr <- c("t1", "t2", "t3")
  paste(
    vapply(pr, pred_single_iqr, character(1),
           contaminante = contaminante, tipo = tipo),
    collapse = " + "
  )
}

# Tipos de modelos
model_types <- c("single", "pct1_t1_t2_t3", "t1_t2_t3")

# Creamos grilla de modelos individuales (single)
combinations_single <- expand.grid(
  dependent = dependent_vars,
  tiempo = time_periods,
  contaminante = contaminants,
  tipo = types,
  model_type = "single",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

# Creamos nombre de predictor para modelos single
combinations_single <- combinations_single |>
  mutate(predictor = purrr::pmap_chr(
    list(tiempo, contaminante, tipo),
    pred_single_iqr
  ))

# Creamos grilla de modelos con pct1 + t1 + t2 + t3
combinations_pct1_t1_t2_t3 <- expand.grid(
  dependent = dependent_vars,
  contaminante = contaminants,
  tipo = types,
  model_type = "pct1_t1_t2_t3",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

combinations_pct1_t1_t2_t3 <- combinations_pct1_t1_t2_t3 |>
  mutate(
    predictor = purrr::pmap_chr(
      list(contaminante, tipo),
      \(co, tp) pred_joint_iqr_pct(co, tp)
    ),
    tiempo = "pct1_t1_t2_t3"
  )

# Creamos grilla de modelos con t1 + t2 + t3
combinations_t1_t2_t3 <- expand.grid(
  dependent = dependent_vars,
  contaminante = contaminants,
  tipo = types,
  model_type = "t1_t2_t3",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

combinations_t1_t2_t3 <- combinations_t1_t2_t3 |>
  mutate(
    predictor = purrr::pmap_chr(
      list(contaminante, tipo),
      \(co, tp) pred_joint_iqr_t123(co, tp)
    ),
    tiempo = "t1_t2_t3"
  )

# Combinamos todas las grillas
combinations <- bind_rows(
  combinations_single,
  combinations_pct1_t1_t2_t3,
  combinations_t1_t2_t3
)

# Predictores válidos: PM25 en escala IQR (iqr_*); Levo/K en nivel bruto (sin iqr_/sin _10)
iqr_pm25_preds <- names(data_model)[
  grepl("^iqr_(pct1|t1|t2|t3|w20|tot)_", names(data_model)) &
    grepl("_PM25_", names(data_model))
]
raw_levo_k_preds <- names(data_model)[
  grepl("^(pct1|t1|t2|t3|w20|tot)_(Levo|K)_", names(data_model)) &
    !grepl("^iqr_", names(data_model)) &
    !grepl("_10$", names(data_model))
]
available_predictors <- union(iqr_pm25_preds, raw_levo_k_preds)

# Para modelos single, verificamos que el predictor exista
combinations_single_valid <- combinations |>
  filter(model_type == "single") |>
  filter(predictor %in% available_predictors)

# Para modelos múltiples, verificamos que todos los predictores existan
combinations_multi <- combinations |>
  filter(model_type != "single") |>
  rowwise() |>
  mutate(
    predictors_list = list(stringr::str_split(predictor, " \\+ ")[[1]]),
    all_exist = all(trimws(predictors_list) %in% available_predictors)
  ) |>
  ungroup() |>
  filter(all_exist) |>
  select(-predictors_list, -all_exist)

# Combinamos resultados válidos
combinations <- bind_rows(combinations_single_valid, combinations_multi)

# Guardamos la grilla de modelos
writexl::write_xlsx(
  combinations, 
  path = "Output/Models/List_models_exposure_malf.xlsx"
)

## 4. Función para estimar modelos logísticos ----

fit_logit_model <- function(dependent, predictor, tiempo, contaminante, tipo, 
                           model_type, data, conf.level = 0.95, adjustment = "Adjusted") {
  
  # Extraemos lista de predictores individuales
  if (model_type == "single") {
    predictors_list <- predictor
  } else {
    predictors_list <- trimws(stringr::str_split(predictor, " \\+ ")[[1]])
  }
  
  # Verificamos que todos los predictores existan en los datos
  missing_predictors <- predictors_list[!predictors_list %in% names(data)]
  if (length(missing_predictors) > 0) {
    return(data.frame(
      term = predictor,
      estimate = NA_real_,
      std.error = NA_real_,
      statistic = NA_real_,
      p.value = NA_real_,
      conf.low = NA_real_,
      conf.high = NA_real_,
      dependent_var = dependent,
      predictor = predictor,
      tiempo = tiempo,
      contaminante = contaminante,
      tipo = tipo,
      model_type = model_type,
      adjustment = adjustment,
      n = 0
    ))
  }
  
  # Filtramos datos con valores válidos en variable dependiente y todos los predictores
  data_subset <- data |>
    dplyr::filter(!is.na(.data[[dependent]]))
  
  # Verificamos valores válidos en todos los predictores
  for (pred in predictors_list) {
    data_subset <- data_subset |>
      dplyr::filter(!is.na(.data[[pred]]))
  }
  
  # Si no hay datos suficientes, retornamos NA
  if (nrow(data_subset) < 10) {
    return(data.frame(
      term = predictor,
      estimate = NA_real_,
      std.error = NA_real_,
      statistic = NA_real_,
      p.value = NA_real_,
      conf.low = NA_real_,
      conf.high = NA_real_,
      dependent_var = dependent,
      predictor = predictor,
      tiempo = tiempo,
      contaminante = contaminante,
      tipo = tipo,
      model_type = model_type,
      adjustment = adjustment,
      n = nrow(data_subset)
    ))
  }
  
  # Construimos fórmula según ajuste
  if (identical(adjustment, "Adjusted")) {
    # Verificamos que las variables de control existan
    available_controls <- control_vars[control_vars %in% names(data_subset)]
    
    rhs <- if (length(available_controls) > 0) {
      paste(
        paste(predictors_list, collapse = " + "),
        paste("+", paste(available_controls, collapse = " + "))
      )
    } else {
      paste(predictors_list, collapse = " + ")
    }
  } else {
    rhs <- paste(predictors_list, collapse = " + ")
  }
  
  # Construimos fórmula
  fml <- as.formula(paste0(dependent, " ~ ", rhs))
  
  # Estimamos modelo
  model_fit <- tryCatch({
    glm(fml, data = data_subset, family = binomial(link = "logit"))
  }, error = function(e) {
    return(NULL)
  })
  
  if (is.null(model_fit)) {
    return(data.frame(
      term = predictor,
      estimate = NA_real_,
      std.error = NA_real_,
      statistic = NA_real_,
      p.value = NA_real_,
      conf.low = NA_real_,
      conf.high = NA_real_,
      dependent_var = dependent,
      predictor = predictor,
      tiempo = tiempo,
      contaminante = contaminante,
      tipo = tipo,
      model_type = model_type,
      adjustment = adjustment,
      n = nrow(data_subset)
    ))
  }
  
  # Extraemos resultados
  tbl <- broom::tidy(model_fit, conf.int = FALSE, exponentiate = FALSE)
  z <- qnorm(1 - (1 - conf.level) / 2)
  
  # Filtramos solo los términos de exposición (predictores)
  tbl_exposure <- tbl[tbl$term %in% predictors_list, ]
  
  if (nrow(tbl_exposure) > 0) {
    tbl_exposure <- tbl_exposure |>
      dplyr::mutate(
        or = exp(estimate),
        conf.low = exp(estimate - z * std.error),
        conf.high = exp(estimate + z * std.error),
        estimate = or,
        dependent_var = dependent,
        predictor = predictor,
        tiempo = tiempo,
        contaminante = contaminante,
        tipo = tipo,
        model_type = model_type,
        adjustment = adjustment,
        n = nrow(data_subset)
      ) |>
      dplyr::select(term, estimate, std.error, statistic, p.value, 
                    conf.low, conf.high, dependent_var, predictor, 
                    tiempo, contaminante, tipo, model_type, adjustment, n)
  } else {
    # Si no encontramos términos de exposición, creamos fila con NAs
    tbl_exposure <- data.frame(
      term = predictors_list[1],
      estimate = NA_real_,
      std.error = NA_real_,
      statistic = NA_real_,
      p.value = NA_real_,
      conf.low = NA_real_,
      conf.high = NA_real_,
      dependent_var = dependent,
      predictor = predictor,
      tiempo = tiempo,
      contaminante = contaminante,
      tipo = tipo,
      model_type = model_type,
      adjustment = adjustment,
      n = nrow(data_subset)
    )
  }
  
  rm(model_fit); gc()
  
  return(tbl_exposure)
}

## 5. Estimamos todos los modelos de forma paralela ----

plan(multisession, workers = parallel::detectCores() - 4)
options(future.globals.maxSize = 1.5 * 1024^3)

tic()
results_list <- future_lapply(seq_len(nrow(combinations)), function(i) {
  message("Iteración ", i, "/", nrow(combinations), " en PID ", Sys.getpid())
  
  dep <- combinations$dependent[i]
  tiempo <- combinations$tiempo[i]
  contaminante <- combinations$contaminante[i]
  tipo <- combinations$tipo[i]
  pred <- combinations$predictor[i]
  model_type <- combinations$model_type[i]
  adj <- combinations$adjustment[i]
  
  fit_logit_model(
    dependent = dep,
    predictor = pred,
    tiempo = tiempo,
    contaminante = contaminante,
    tipo = tipo,
    model_type = model_type,
    data = data_model,
    adjustment = adj
  )
}, future.seed = TRUE)
toc() # 22 sec

plan(sequential)

## 6. Consolidamos y guardamos resultados ----

results_logit <- dplyr::bind_rows(results_list)

# Guardamos resultados
save(results_list, file = "Output/Models/Exposure_models_malf.RData")

writexl::write_xlsx(
  results_logit, 
  path = "Output/Models/Exposure_models_malf.xlsx"
)

# Test de prueba de algunos modelos 
m1 <- glm(
  malf ~ iqr_pct1_PM25_cs + iqr_t1_PM25_cs + iqr_t2_PM25_cs + iqr_t3_PM25_cs +
    pct1_Levo_cs + t1_Levo_cs + t2_Levo_cs + t3_Levo_cs,
  # edad_madre + sexo_rn + a_nac + estacion,
  family = binomial(link = logit), data = data_full_wide)

summary(m1)
exp(m1$coefficients[-1])
exp(confint.default(m1))

AIC(m1)
BIC(m1)

m2 <- glm(
  malf ~ iqr_t1_PM25_cs + iqr_t2_PM25_cs + iqr_t3_PM25_cs +
  edad_madre + sexo_rn + a_nac + estacion, 
  family = binomial(link = logit), data = data_full_wide)

summary(m2)
exp(m2$coefficients[-1])
exp(confint.default(m2)) 

AIC(m2)
BIC(m2)

m3 <- glm(
  malf ~ iqr_tot_PM25_cs +
  edad_madre + sexo_rn + a_nac + estacion, 
  family = binomial(link = logit), data = data_full_wide)

summary(m3)
exp(m3$coefficients[-1])
exp(confint.default(m3)) 

AIC(m3)
BIC(m3)

# Extract information models 
m1_tidy <- broom::tidy(m1, conf.int = TRUE, exponentiate = TRUE)
m2_tidy <- broom::tidy(m2, conf.int = TRUE, exponentiate = TRUE)
m3_tidy <- broom::tidy(m3, conf.int = TRUE, exponentiate = TRUE)

data_full_wide <- data_full_wide |> 
  mutate(
    mes_nac = month(fecha_nac)
  )
glimpse(data_full_wide)


m2b <- glm(
  malf ~ iqr_t1_PM25_cs + iqr_t2_PM25_cs + iqr_t3_PM25_cs +
  edad_madre + sexo_rn + a_nac + estacion + comuna, 
  family = binomial(link = logit), data = data_full_wide)

summary(m2)
summary(m2b)
exp(m2$coefficients[-1])
exp(m2b$coefficients[-1])
exp(confint.default(m2)) 
exp(confint.default(m2b)) 

AIC(m2)
AIC(m2b)

BIC(m2)
BIC(m2b)

m3b <- glm(
  malf ~ iqr_tot_PM25_cs +
  edad_madre + sexo_rn + a_nac + estacion + comuna, 
  family = binomial(link = logit), data = data_full_wide)

summary(m3)
summary(m3b)
exp(m3$coefficients[-1])
exp(m3b$coefficients[-1])
exp(confint.default(m3)) 
exp(confint.default(m3b)) 

AIC(m3)
AIC(m3b)
BIC(m3)
BIC(m3b)

## 7. Preparamos datos y grillas: malf_card_bin (población general vs cardiopatías; sin otros malf) ----
#
# Muestra: controles (sin malformación) + casos con malformación cardiaca.
# Se excluyen los casos con malformación no cardiaca (malf == 1 y malf_card_bin == 0).

data_model_card <- data_full_wide |>
  filter(!is.na(malf_card_bin)) |>
  filter(!is.na(malf)) |>
  filter(!(malf == 1 & malf_card_bin == 0))

dependent_vars_card <- "malf_card_bin"

## 8. Definimos variables y creamos grilla de modelos (solo malf_card_bin) ----

combinations_single_card <- expand.grid(
  dependent = dependent_vars_card,
  tiempo = time_periods,
  contaminante = contaminants,
  tipo = types,
  model_type = "single",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

combinations_single_card <- combinations_single_card |>
  mutate(predictor = purrr::pmap_chr(
    list(tiempo, contaminante, tipo),
    pred_single_iqr
  ))

combinations_pct1_t1_t2_t3_card <- expand.grid(
  dependent = dependent_vars_card,
  contaminante = contaminants,
  tipo = types,
  model_type = "pct1_t1_t2_t3",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

combinations_pct1_t1_t2_t3_card <- combinations_pct1_t1_t2_t3_card |>
  mutate(
    predictor = purrr::pmap_chr(
      list(contaminante, tipo),
      \(co, tp) pred_joint_iqr_pct(co, tp)
    ),
    tiempo = "pct1_t1_t2_t3"
  )

combinations_t1_t2_t3_card <- expand.grid(
  dependent = dependent_vars_card,
  contaminante = contaminants,
  tipo = types,
  model_type = "t1_t2_t3",
  adjustment = c("Unadjusted", "Adjusted"),
  stringsAsFactors = FALSE
)

combinations_t1_t2_t3_card <- combinations_t1_t2_t3_card |>
  mutate(
    predictor = purrr::pmap_chr(
      list(contaminante, tipo),
      \(co, tp) pred_joint_iqr_t123(co, tp)
    ),
    tiempo = "t1_t2_t3"
  )

combinations_card <- bind_rows(
  combinations_single_card,
  combinations_pct1_t1_t2_t3_card,
  combinations_t1_t2_t3_card
)

iqr_pm25_card <- names(data_model_card)[
  grepl("^iqr_(pct1|t1|t2|t3|w20|tot)_", names(data_model_card)) &
    grepl("_PM25_", names(data_model_card))
]
raw_levo_k_card <- names(data_model_card)[
  grepl("^(pct1|t1|t2|t3|w20|tot)_(Levo|K)_", names(data_model_card)) &
    !grepl("^iqr_", names(data_model_card)) &
    !grepl("_10$", names(data_model_card))
]
available_predictors_card <- union(iqr_pm25_card, raw_levo_k_card)

combinations_single_valid_card <- combinations_card |>
  filter(model_type == "single") |>
  filter(predictor %in% available_predictors_card)

combinations_multi_card <- combinations_card |>
  filter(model_type != "single") |>
  rowwise() |>
  mutate(
    predictors_list = list(stringr::str_split(predictor, " \\+ ")[[1]]),
    all_exist = all(trimws(predictors_list) %in% available_predictors_card)
  ) |>
  ungroup() |>
  filter(all_exist) |>
  select(-predictors_list, -all_exist)

combinations_card <- bind_rows(combinations_single_valid_card, combinations_multi_card)

writexl::write_xlsx(
  combinations_card,
  path = "Output/Models/List_models_exposure_malf_card_bin.xlsx"
)

## 9. Estimamos modelos logísticos malf_card_bin en paralelo ----

plan(multisession, workers = parallel::detectCores() - 4)
options(future.globals.maxSize = 1.5 * 1024^3)

tic()
results_list_card <- future_lapply(seq_len(nrow(combinations_card)), function(i) {
  message("Iteración card ", i, "/", nrow(combinations_card), " en PID ", Sys.getpid())

  dep <- combinations_card$dependent[i]
  tiempo <- combinations_card$tiempo[i]
  contaminante <- combinations_card$contaminante[i]
  tipo <- combinations_card$tipo[i]
  pred <- combinations_card$predictor[i]
  model_type <- combinations_card$model_type[i]
  adj <- combinations_card$adjustment[i]

  fit_logit_model(
    dependent = dep,
    predictor = pred,
    tiempo = tiempo,
    contaminante = contaminante,
    tipo = tipo,
    model_type = model_type,
    data = data_model_card,
    adjustment = adj
  )
}, future.seed = TRUE)
toc()

plan(sequential)

## 10. Consolidamos y guardamos resultados malf_card_bin ----

results_logit_card <- dplyr::bind_rows(results_list_card)

save(results_list_card, file = "Output/Models/Exposure_models_malf_card_bin.RData")

writexl::write_xlsx(
  results_logit_card,
  path = "Output/Models/Exposure_models_malf_card_bin.xlsx"
)
