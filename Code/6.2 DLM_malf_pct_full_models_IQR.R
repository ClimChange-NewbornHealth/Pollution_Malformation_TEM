# Code 6.2 DLM Malformaciones - Full Models PCT (exposición IQR, desfase +13 como script original) ----
# Misma lógica que 6.1 salvo week interno +13 y Week reportada = semana interna − 13.
# Figuras: 2 paneles (malf y malf_card_bin), 2×3 (filas cs/sp, columnas contaminantes).
...
data_full_long <- data_wide |>
  ...
  dplyr::mutate(
    week = as.integer(sub("^w", "", tiempo)) + 13L
  ) |>
  dplyr::arrange(malf, idbase, week)

# PM2.5 en escala /10 (weekly), eje interno +13 como el resto del script PCT ----
...
process_dlm_model <- function(dep_var, contam, tipo_val, data_long, control_vars) {
  ...
  common_weeks <- common_weeks[common_weeks >= 2 & common_weeks <= 52]
...
trimester_bands_pct <- tibble::tribble(
  ~xmin, ~xmax, ~fill_col,
  0, 13, "gray70",
  13, 27, "white",
  27, Inf, "gray70"
...
# Zona sin sombreado antes de semana 0: etiqueta "PCT" en la banda blanca
x_lim_pct_min <- -11.5
pct_strip_label <- tibble::tibble(
  x = (x_lim_pct_min + 0) / 2,
  y = Inf,
  label = "PCT"
)
...
plot_dlm_single_pct <- function(data, contam_code, panel_title) {
  ...
  for (tipo_val in tipos) {
    for (j in seq_along(contaminantes)) {
      ...
      if (is.null(res)) {
        ...
        plots_list[[length(plots_list) + 1L]] <- plot_dlm_single_pct(d, contam, tit)
      }
    }
  }
...
out_dir <- "Output/DLM/PCT"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

out_xlsx <- file.path(out_dir, "Malf_DLM_results_PCT_models_IQR.xlsx")
...
if (length(all_results) > 0) {
  for (dv in dependent_vars) {
    ...
    fn <- file.path(out_dir, paste0("Malf_DLM_IQR_PCT_panel_", dv, ".png"))
    ggplot2::ggsave(
      fn,
      plot = p_out,
      res = 300,
