# 5.3 Exposure models IQR estratificados por sexo del RN -----
# Misma grilla que 5.1 con outcomes malf y malf_card_bin; covariables de ajuste sin sexo_rn dentro de cada estrato.
# Excel por sexo con la misma estructura que 5.2: Table 2 (single), Mutual t1+t2+t3, Mutual pct1+t1+t2+t3, Footnote.
...
## 3. Variables y grillas (solo malf y malf_card_bin; misma estructura que 5.1) ----
...
make_combinations <- function(dependent_vec) {
  ...
  combinations_single <- combinations_single |>
    mutate(predictor = purrr::pmap_chr(list(tiempo, contaminante, tipo), pred_single_iqr))
  ...
  combinations_pct1_t1_t2_t3 <- combinations_pct1_t1_t2_t3 |>
    mutate(
      predictor = purrr::pmap_chr(list(contaminante, tipo), \(co, tp) pred_joint_iqr_pct(co, tp)),
  ...
  combinations_t1_t2_t3 <- combinations_t1_t2_t3 |>
    mutate(
      predictor = purrr::pmap_chr(list(contaminante, tipo), \(co, tp) pred_joint_iqr_t123(co, tp)),
      tiempo = "t1_t2_t3"
    )
...
writexl::write_xlsx(
  ...
  path = "Output/Models/List_models_exposure_malf_iqr_by_sex.xlsx"
)
...
## 5–6. Por sexo: estimamos malf y malf_card_bin; guardamos resultados y tablas ----
...
period_from_metric_term <- function(term) {
  m <- stringr::str_match(term, "^iqr_(pct1|t1|t2|t3)_")
  out <- ifelse(!is.na(m[, 1L]), m[, 2L], NA_character_)
  miss <- is.na(out)
  if (!any(miss)) return(out)
  m2 <- stringr::str_match(term[miss], "^(pct1|t1|t2|t3)_(PM25|Levo|K)_")
  out[miss] <- ifelse(is.na(m2[, 1L]), NA_character_, m2[, 2L])
  out
}
...
build_joint_template_table <- function(results_df, dependent_target, model_type_val, periods_vec) {
  d <- results_df |>
    ...
    dplyr::mutate(period_key = period_from_metric_term(.data$term)) |>
...
footnote_single <- paste(
  "Adjusted logistic models (OR as RR); PM2.5 per IQR; Levo and K raw exposure level.",
...
footnote_joint_t123 <- paste(
  "Adjusted logistic models (OR as RR); PM2.5 per IQR; Levo/K raw.",
  "Stratified by infant sex (female/male); sex excluded from adjustment within stratum.",
  "Adjustment: maternal age, year of birth, season of conception.",
  "One model per pollutant/type: mutually adjusted t1, t2, t3 exposures (mixed scaling as specified)."
)
...
footnote_joint_pct <- paste(
  ...
  "Stratified by infant sex (female/male); sex excluded from adjustment within stratum.",
  "Adjustment: maternal age, year of birth, season of conception.",
  "One model per pollutant/type: mutually adjusted pct1, t1, t2, t3 (mixed scaling as specified)."
)
...
for (sx in sex_levels) {
  ...
  save(results_list, file = file.path(out_dir_models, paste0("Exposure_models_malf_iqr_", sfx, ".RData")))
  writexl::write_xlsx(
    results_malf_df,
    path = file.path(out_dir_models, paste0("Exposure_models_malf_iqr_", sfx, ".xlsx"))
  )
  ...
  writexl::write_xlsx(
    ...
    path = file.path(out_dir_tables, paste0("Table_IQR_adjusted_malf_sex_", sfx, ".xlsx"))
  )
  ...
  save(results_list_card, file = file.path(out_dir_models, paste0("Exposure_models_malf_card_bin_iqr_", sfx, ".RData")))
  writexl::write_xlsx(
    results_card_df,
    path = file.path(out_dir_models, paste0("Exposure_models_malf_card_bin_iqr_", sfx, ".xlsx"))
  )
  ...
  writexl::write_xlsx(
    ...
    path = file.path(out_dir_tables, paste0("Table_IQR_adjusted_malf_card_sex_", sfx, ".xlsx"))
  )
}
